...
// ON_MESSAGE(WM_NET_CLIENT_CONNECTED, &CDialogNetCfg::OnClientConnected)
// ON_MESSAGE(WM_NET_CLIENT_DISCONNECTED, &CDialogNetCfg::OnClientDisconnected)
		...
		else
		{
			...
			else
			{
				...
				if (m_pClientSocket->Connect(m_ServerIP, m_ServerPortNumber) == 0)
				{
					...
					// sprintf_s(szError, 256, "Listen Failed: %d", GetLastError());
